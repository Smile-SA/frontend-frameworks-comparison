...
const solutions = ref<ISolution[]>([])
onMounted(() => {
  fetch('http://localhost:3333/solutions')
    .then((r) => r.json())
    .then((json) => (solutions.value = json))
})

const technologies = ref<ISolution[]>([])
onMounted(() => {
  fetch('http://localhost:3333/technologies')
    .then((r) => r.json())
    .then((json) => (technologies.value = json))
})
      ...
      <div class="js-tabs ui-tabs ui-corner-all ui-widget ui-widget-content">
        ...
        >
          <div class="paragraph paragraph--type--expertise paragraph--view-mode--default">
            <div v-if="solutions" class="row">
              <template v-for="(item, i) of solutions">
                ...
                <div v-if="i % 2 === 1" class="clearfix hidden-md hidden-lg hidden-xl"></div>
              </template>
            </div>
          </div>
        </div>
        ...
        >
          <div class="paragraph paragraph--type--expertise paragraph--view-mode--default">
            <div v-if="technologies" class="row">
              <template v-for="(item, i) of technologies">
                <div class="field__item col-xs-12 col-sm-6 col-md-3">
                  <a
                    class="paragraph paragraph--type--expertise-item paragraph--view-mode--default"
                    :href="item.url"
