...
const openSources = ref<IOpenSource[]>([])
onMounted(() => {
  fetch('http://localhost:3333/opensource')
    .then((r) => r.json())
    .then((json) => (openSources.value = json))
})
</script>
<template>
  <div class="field--item">
    <div class="block-content block-content-two_parts big onepartright margin block-twoparts">
      <div
        v-if="openSources && openSources[0]"
      ...
      >
        <div class="singleblock-text">
          <h2 class="node-title">{{ openSources[0].title }}</h2>
      ...
      <div
        v-if="openSources && openSources[1]"
        class="single-block white_paper block_single_part_one_third_tall blue"
        :style="{
          backgroundColor: '#FFFFFF',
          backgroundImage: 'url(https://www.smile.eu/sites/default/files/2017-09/bg_gris.png)',
          backgroundSize: 'cover',
          backgroundRepeat: 'no-repeat',
          backgroundPosition: 'center center'
        }"
