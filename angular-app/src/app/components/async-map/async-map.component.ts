...
@Component({
  selector: 'app-async-map',
  templateUrl: './async-map.component.html',
  styleUrls: ['./async-map.component.css'],
})
export class AsyncMapComponent implements AfterViewInit {
  constructor(private http: HttpClient) {}

  ngAfterViewInit(): void {
    try {
      if (typeof window !== 'undefined') {
        import('leaflet').then((module) => this.attach(module));
      }
    } catch (e) {
      console.error(e);
    }
  ...
  attach(L: any) {
    ...
    const mapUri =
      'assets/smileeu_local_map/pictures_map_global/{z}/{x}/{y}.png';
    ...
    if (drupalSettings.smileeu.map.taxonomy[1]) {
      if ((window.innerWidth > 480 && isFront) || !isFront) {
        this.http
          .get<FeatureCollection<Point>>('http://localhost:3333/agencies')
          .subscribe((agencies) => this.addDataToMap(L, agencies, map));
      ...
      if (window.innerWidth <= 480 && isFront) {
        this.disableControl(map);
      }
    }
  }

  disableControl(map: Map) {
