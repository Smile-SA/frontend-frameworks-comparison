...
function attach() {
  ...
  const zoomDelta = 1;
  let zoomSnap = 1;
  let taxonomy = drupalSettings.smileeu.map.taxonomy;
  const isFront = drupalSettings.smileeu.map.isFront;
  let rest = "/" + drupalSettings.path.pathPrefix + "agencies/" + taxonomy;
  const mapUri = "/smileeu_local_map/pictures_map_global/{z}/{x}/{y}.png";
  ...
  if (drupalSettings.smileeu.map.taxonomy[1]) {
    taxonomy = drupalSettings.smileeu.map.taxonomy;
    rest = "/" + drupalSettings.path.pathPrefix + "agencies/" + taxonomy;
    if ((window.innerWidth > 480 && isFront) || !isFront) {
      // jQuery.getJSON(rest, function (data) {
      //   if (drupalSettings.smileeu.nids) {
      //     addDataToMap(data, maps[0], drupalSettings, $(window));
      //   } else {
      //     addDataToMap(data, map, drupalSettings, $(window));
      //   }
      // });
...
function addDataToMap(data: typeof agencies, map: Map) {
